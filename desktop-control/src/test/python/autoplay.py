...
def outcome_of(state):
    ...
    if not isinstance(outcome, dict):
        return None
    if isinstance(outcome.get("scope_id"), str) and outcome["scope_id"] and outcome["scope_id"] == state.get("scope_id") and outcome.get("result") in {"won", "lost"}:
...
class PublicClient(Client):
    ...
    def request(self, op, args=None, request_id=None, scope=None, version=None):
        ...
        if protocol6.is_live_operation(op) and response.get("ok") and isinstance(data, dict) and "observation" in data:
        ...
        for log in sorted(attempt.glob("strategy-public-*.ndjson"), key=lambda p: p.stat().st_mtime):
            with log.open(encoding="utf-8") as source:
                for line in source:
                    ...
                    decoded = protocol6.response(wire_response) if wire_response.get("v") == 5 else wire_response
