...
class TextSensitiveMatrixTest(unittest.TestCase):
    ...
    def test_first_protocol_error_stops_later_requests_before_the_transport(self):
        ...
        failed = {"protocol_version": 2, "ok": False, "error": {"code": "STALE_STATE"}}
    ...
    def test_event_lists_receive_source_validation_and_clipped_text_is_allowed(self):
        client, evidence = self.fake_client()
        missing = {"protocol_version": 2, "ok": True, "result": [{"text": "Unclassified engine text"}]}
        ...
        client, evidence = self.fake_client()
        clipped = {"protocol_version": 2, "ok": True, "result": [{"text": "Partially displayed text", "clipped": True,
        ...
        with patch.object(matrix.MatrixClient, "request", return_value=clipped):
            self.assertEqual(clipped, client.request("events.read"))
    ...
    def test_actual_gui_language_must_match_the_worker(self):
        client, evidence = self.fake_client()
        wrong = {"protocol_version": 2, "ok": True, "result": {"observation": {"ui": {
