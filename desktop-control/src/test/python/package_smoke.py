...
def receive(process, timeout=40):
    ...
    while not data.endswith(b"\n"):
        ...
        if select.select([process.stdout], [], [], min(remaining, 1))[0]:
            ...
            data.extend(chunk)
    return protocol4.response(json.loads(data))
...
def main():
    ...
    output = root / "desktop-control/build/fixtures/packaging4.0" / ("raw-" + uuid.uuid4().hex)
    ...
    try:
        ...
        first = exchange(b'{"v":4,"id":"package-info","op":"info"}\r\n')
        assert first["ok"], first
        scope = first["result"]["scope_id"]
        invalid = exchange(b'{"v":4,"id":"invalid-encoding","op":"state","x":"\xff"}\n')
        assert invalid["error"]["code"] == "INVALID_ENCODING", invalid
        query = protocol4.wire_bytes(protocol4.request("state.get", request_id="中文-query", scope=scope))
        ...
        if process.poll() is None:
            ...
            process.wait()
        stderr.close()
    final_frame=protocol4.wire_bytes(protocol4.request("state.get", request_id="package-eof-frame", scope=scope))[:-1]
    ...
    assert len(delivered)==1 and protocol4.response(json.loads(delivered[0]))["ok"],delivered
