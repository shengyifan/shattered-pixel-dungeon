#!/usr/bin/env python3
"""Deep protocol-2 text cases in independent processes and disposable profiles.
    ...
    class SensitiveMatrixClient(MatrixClient):
        ...
        def request(self, op, args=None, **kwargs):
            ...
            try:
                ...
                if op == "protocol.info":
                    assert result["protocol_version"] == 2 and result["audit_schema_version"] == 5, result
                    assert result["cli_version"] == "CLI.2.0.0" and result["text_language"] == "en", result
                    assert result.get("build_id") and result.get("session_id"), result
                    metadata = {key: result[key] for key in ("build_id", "cli_version", "protocol_version", "audit_schema_version", "text_language", "session_id")}
...
def run_worker(job_path):
    ...
    try:
        ...
        assert evidence.handshakes and evidence.source_checks > 0, "No protocol-2 source evidence was recorded"
