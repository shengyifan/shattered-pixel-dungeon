#!/usr/bin/env python3
"""Deep protocol-5 text cases in independent processes and disposable profiles.
    ...
    class SensitiveMatrixClient(MatrixClient):
        ...
        def request(self, op, args=None, **kwargs):
            ...
            try:
                ...
                if op == "protocol.info":
                    assert response["protocol_version"] == 5 and result["audit_schema_version"] == 8, result
                    assert result["cli_version"] == "CLI.5.0.0" and result["text_language"] == "en", result
...
def run_worker(job_path):
    ...
    try:
        ...
        assert evidence.handshakes and evidence.source_checks > 0, "No protocol-5 source evidence was recorded"
