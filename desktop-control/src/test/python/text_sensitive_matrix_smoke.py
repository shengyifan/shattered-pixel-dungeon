#!/usr/bin/env python3
"""Deep protocol-6 text cases in independent processes and disposable profiles.
    ...
    class SensitiveMatrixClient(MatrixClient):
        ...
        def request(self, op, args=None, **kwargs):
            ...
            try:
                ...
                if op == "protocol.info":
                    assert response["protocol_version"] == 6 and result["audit_schema_version"] == 9, result
                    assert result["cli_version"] == "CLI.6.0.0" and result["text_language"] == "en", result
...
def run_worker(job_path):
    ...
    try:
        ...
        assert evidence.handshakes and evidence.source_checks > 0, "No protocol-6 source evidence was recorded"
