...
class Protocol8BaseTest(unittest.TestCase):
    def test_semantic_cue_lists_keep_their_inner_key_in_live_states_and_events(self):
        cues = [{"kind": "bomb_smoke", "cell": 6},
                {"kind": "blast_wave", "cell": 6, "appearance": {
                    "shape": "ring", "cells": [6, 7], "coverage": "visual_extent", "partial": True}}]
        snapshot = {"status": "last_observed", "depth": 10, "map_context": "fixture:tengu", "cues": cues}
        wire = {"v": 8, "id": "state", "st": "completed", "data": {"scene": "game", "cues": snapshot}}
        before = copy.deepcopy(wire)
        observation = protocol8.response(wire, "state")["result"]["observation"]
        ...
        for visible in (cues, []):
            event_data = {"format": "semantic_cue_snapshot_v1", "depth": 10,
                          "map_context": "fixture:tengu", "cues": visible}
            event = {"v": 8, "id": "event", "st": "completed", "data": {
                "items": [{"kind": "game.visual", "data": event_data}], "next": None, "end": True, "until": 1}}
            decoded = protocol8.response(event, "events")["result"][0]
            self.assertEqual(event_data, decoded["data"])

    def test_aliases_defaults_semantic_item_display_and_source_ast_are_separate(self):
        ast = {"kind": "external", "max_hp": 99, "value": "unmodified source keys"}
        wire = {"v": 8, "id": "q", "s": "run:1", "rev": "r5", "st": "completed", "data": {
            "scene": "game", "hero": {"hp": 24, "ht": 30, "experience": 10, "mxp": 20, "tp": [0, 0, 0, 0]},
            "inv": [{"loc": "backpack.0", "name": "unknown armor", "level": None,
                     "cursed": None, "shown": {"extra": "14?"}}],
            "ui": {"nodes": [{"id": "slot", "subject": {"kind": "item", "loc": "backpack.0"},
                               "text": "14?", "text_sources": {"text": ast},
                               "ops": [{"op": "click"}]}]},
            "persistence": {"saves": [{"sid": "s1", "src_s": "run:1", "src_id": "a1", "at": "time", "success": True}]}}}
        before = copy.deepcopy(wire)
        state = protocol8.response(wire)["result"]
        ...
        self.assertEqual(ast, obs["ui"]["controls"][0]["text_sources"]["text"])
        self.assertEqual("14?", obs["inventory"][0]["shown"]["extra"])
        self.assertEqual("s1", state["persistence"]["saves_during_request"][0]["receipt_id"])
        self.assertEqual(before, wire)
        with self.assertRaises(AssertionError):
            protocol8.response({"v": 4, "st": "completed"})
    ...
    def test_per_frame_entity_and_effect_dictionaries_expand_without_history(self):
        ...
        obs = protocol8.state(data)["observation"]
        ...
        new = protocol8.state(second)["observation"]
    ...
    def test_missing_invalid_or_overriding_dictionary_references_fail(self):
        for bad in (-1, 1, True, 0.0, "0"):
            with self.subTest(ref=bad), self.assertRaises(AssertionError):
                protocol8.state({"scene": "game", "entity_defs": [{"kind": "trap"}], "entities": [{"cell": 1, "def": bad}]})
        with self.assertRaises(AssertionError):
            protocol8.state({"scene": "game", "entities": [{"cell": 1, "def": 0}]})
        with self.assertRaises(AssertionError):
            protocol8.state({"scene": "game", "entity_defs": [{"kind": "trap"}], "entities": [{"cell": 1, "def": 0, "name": "override"}]})
        for value in (0, True, "0"):
            with self.subTest(effect=value), self.assertRaises(AssertionError):
                protocol8.state({"scene": "game", "map": {"w": 1, "h": 1, "types": [{"terrain": 1}], "rows": [[0, 0, "0", "v"]], "env": {"0": value}}})

    def test_uniform_visibility_integer_tiles_and_invalid_lengths(self):
        types = [{"terrain": i} for i in range(65)]
        obs = protocol8.state({"scene": "game", "map": {"w": 5, "h": 1, "types": types, "rows": [[0, 1, [64, 0, 5], "m"]]}})["observation"]
        self.assertEqual(["mapped"] * 3, [c["visibility"] for c in obs["map"]["cells"]])
        for tiles, vis in (("", "v"), ("000", "vs"), ("00", "")):
            with self.assertRaises(AssertionError):
                protocol8.state({"scene": "game", "map": {"w": 5, "h": 1, "types": [{}], "rows": [[0, 0, tiles, vis]]}})

    def test_actions_are_flat_and_directions_use_wire_enum(self):
        wire = protocol8.request("action.execute", {"action": "move.step", "direction": "northeast"}, "a", "run:1", "r7")
        self.assertEqual({"v": 8, "id": "a", "s": "run:1", "rev": "r7", "op": "move", "dir": "NE"}, wire)
        self.assertNotIn("args", wire)
        self.assertNotIn("action", wire)
        self.assertEqual("click", protocol8.request("action.execute", {"action": "ui.activate", "control": "n1"})["op"])
    ...
    def test_direct_short_action_preserves_revision_and_updates_live_context(self):
        ...
        client.buffer = protocol8.wire_bytes({"v": 8, "id": "a", "s": "run:1", "rev": "r8", "st": "completed",
                                            "data": {"scene": "game", "phase": "player_ready"}})
        response = client.request("move", {"dir": "N"}, request_id="a", version="r7")
        self.assertEqual({"v": 8, "id": "a", "s": "run:1", "rev": "r7", "op": "move", "dir": "N"},
        ...
        self.assertFalse(protocol8.is_live_operation("req"))
        self.assertFalse(protocol8.is_live_operation("events"))

    def test_info_version_comes_from_envelope_without_inventing_data_field(self):
        wire = {"v": 8, "id": "hello", "s": "menu:1", "rev": "r1", "st": "completed",
                "data": {"cli_version": "CLI.8.0.0", "audit_schema_version": 11}}
        response = protocol8.response(wire, "info")
        self.assertEqual(8, response["protocol_version"])
    ...
    def test_uncertain_error_keeps_confirmed_saves_without_advancing_live_context(self):
        ...
        wire = {"v": 8, "id": "a", "s": "run:1", "rev": "r8", "err": "EXECUTION_UNKNOWN",
    ...
    def test_source_and_request_details_are_opt_in(self):
        self.assertEqual({"v": 8, "id": "q", "op": "state"}, protocol8.request("state.get", request_id="q"))
        self.assertTrue(protocol8.request("state.get", {"src": True})["src"])
        compact = protocol8.request("request.get", {"target_id": "a"})
        self.assertEqual("a", compact["rid"])
        self.assertNotIn("get", compact)
        detail = protocol8.request("request.get", {"target_id": "a", "get": ["reply"]})
        self.assertEqual(["reply"], detail["get"])

    def test_map_and_node_projection_reconstructs_only_public_values(self):
        wire = {"v": 8, "id": "q", "s": "run:1", "rev": "r7", "st": "completed", "data": {
        ...
        decoded = protocol8.response(wire, "state.get")["result"]
    ...
    def test_activity_and_disclosed_run_outcome_keep_canonical_observation_locations(self):
        ...
        wire = {"v": 8, "id": "a", "s": "run:1", "rev": "activity:r1", "st": "in_progress",
                "data": {"scene": "game", "phase": "continuous_activity", "continuous_activity": activity}}
        state = protocol8.response(wire, "cell")["result"]
        ...
        state = protocol8.response(wire, "state")["result"]
    ...
    def test_sources_and_partial_diagnostics_are_not_fabricated(self):
        ...
        wire = {"v": 8, "data": {"scene": "game", "ui": {"nodes": [{"id": "n", "text": "A",
        ...
        action = protocol8.response(wire)["result"]["actions"][0]
        self.assertEqual(source, action["text_sources"]["label"])
        self.assertEqual("clipped_text", action["text_diagnostics"]["label"])
        with self.assertRaises(AssertionError):
            protocol8.response({"protocol_version": 2, "ok": True})
        with self.assertRaises(AssertionError):
            protocol8.response({"v": 3, "st": "completed"})
    ...
    def test_map_integer_overflow_sparse_unknown_and_environment_removal(self):
        ...
        data = {"scene": "game", "map": {"w": 5, "h": 2, "types": types,
                ...
                "env": {"2": [{"name": "gas"}]}}}
        first = protocol8.state(data)["observation"]["map"]
        ...
        del data["map"]["env"]
        second = protocol8.state(data)["observation"]["map"]
    ...
    def test_item_three_states_and_documented_defaults_preserve_known_zero_false(self):
        ...
        items = protocol8.state(data)["observation"]["inventory"]
    ...
    def test_protected_item_knowledge_pairs_are_not_reinferred_from_values(self):
        ...
        decoded = protocol8.state({"scene": "game", "inv": items})["observation"]["inventory"]
    ...
    def test_preserved_cells_stay_opaque_diagnostics_and_never_supply_live_cells(self):
        ...
        decoded = protocol8.state(data)["observation"]["map"]
        ...
        data["map"]["rows"] = []
        empty = protocol8.state(data)["observation"]["map"]
    ...
    def test_action_label_is_independent_of_visible_numeric_item_text(self):
        wire = {"v": 8, "data": {"scene": "game", "ui": {"nodes": [
    ...
    def test_request_summary_does_not_invent_reply_and_details_decode_exact_reply(self):
        summary = {"v": 8, "id": "lookup", "data": {"id": "a", "op": "wait", "st": "COMPLETED", "has": ["reply", "before"]}}
        self.assertNotIn("response", protocol8.response(summary, "request.get")["result"])
        reply = {"v": 8, "id": "a", "s": "run:1", "rev": "r8", "st": "completed", "data": {"scene": "game", "phase": "player_ready"}}
        summary["data"]["reply"] = reply
        self.assertEqual(protocol8.response(reply), protocol8.response(summary, "request.get")["result"]["response"])

    def test_history_metadata_keeps_actual_operation_instead_of_becoming_an_action(self):
        wire = {"v": 8, "id": "h", "data": {"items": [{"sequence": 2, "id": "a", "op": "move", "status": "COMPLETED"}],
                "next": None, "end": True, "until": 2}}
        row = protocol8.response(wire, "history.list")["result"][0]
    ...
    def test_limit_one_self_auditing_history_stops_at_initial_bound(self):
        class GrowingHistory:
            ...
            def request(self, op, args, scope=None):
                ...
                more = sequence < upper
                wire = {"v": 8, "id": "page", "data": {"items": [{"sequence": sequence}],
                        "until": upper, "next": sequence if more else None, "end": not more}}
                return protocol8.response(wire, op)

        client = GrowingHistory()
        rows = [row for batch in protocol8.pages(client, "history.list", limit=1) for row in batch]
        ...
        rows = [row for batch in protocol8.pages(client, "history.list", after=3, limit=1) for row in batch]
    ...
    def test_large_single_request_and_response_are_collected_without_truncation(self):
        engine = 'import json,sys; r=json.loads(sys.stdin.buffer.readline()); print(json.dumps({"v":8,"id":r["id"],"st":"completed","data":{"echo":r["text"]}},ensure_ascii=False),flush=True)'
        ...
        try:
            ...
            self.assertEqual(8, json.loads(client.last_send_bytes)["v"])
