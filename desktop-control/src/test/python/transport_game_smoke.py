#!/usr/bin/env python3
"""Production CLI 4: fresh defaults, passive viewers, exact transport and saved restart.
...
class CapturedClient(Client):
    ...
    def request(self, op, args=None, request_id=None, scope=None, version=None):
        ...
        request = protocol4.request(op, args, request_id or f"{self.prefix}-{self.counter}",
                                    scope or self.scope, version or self.version)
        wire = protocol4.wire_bytes(request)
        ...
        response = protocol4.response(self.last_wire_response, op)
...
def main():
    ...
    output = root / "desktop-control/build/fixtures/packaging4.0" / ("transport-defaults-" + uuid.uuid4().hex)
