#!/usr/bin/env python3
"""Production CLI 5: fresh defaults, passive viewers, exact transport and saved restart.
...
class CapturedClient(Client):
    ...
    def request(self, op, args=None, request_id=None, scope=None, version=None):
        ...
        request = protocol8.request(op, args, request_id or f"{self.prefix}-{self.counter:d}",
                                    scope or self.scope, version or self.version)
        wire = protocol8.wire_bytes(request)
        ...
        response = protocol8.response(self.last_wire_response, op)
...
def fresh_case(cli, bundle, output, env, initial_kind, expected_build, expected_cli):
    ...
    try:
        ...
        saved = execute(client, "game.save").initial_response
...
def main():
    ...
    parser.add_argument("--expected-cli", default="CLI.8.0.0")
    ...
    output = root / "desktop-control/build/fixtures/packaging8.0" / ("transport-defaults-" + uuid.uuid4().hex)
