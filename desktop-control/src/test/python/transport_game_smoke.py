...
class CapturedClient(Client):
    ...
    def request(self, op, args=None, request_id=None, scope=None, version=None):
        ...
        request = protocol5.request(op, args, request_id or f"{self.prefix}-{self.counter}",
                                    scope or self.scope, version or self.version)
        wire = protocol5.wire_bytes(request)
        ...
        response = protocol5.response(self.last_wire_response, op)
...
def main():
    ...
    parser.add_argument("--expected-cli", default="CLI.5.0.0")
    ...
    output = root / "desktop-control/build/fixtures/packaging5.0" / ("transport-defaults-" + uuid.uuid4().hex)
