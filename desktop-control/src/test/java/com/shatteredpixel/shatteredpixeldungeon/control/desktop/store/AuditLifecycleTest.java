...
public class AuditLifecycleTest {
    ...
    @Test public void sessionHistoryDistinguishesCleanCloseInterruptionAndDuplicateArrival()throws Exception{
        ...
        try(AuditStore store=new AuditStore(root)){
            scope=store.menuScope();first=store.beginSession("boot-1","fixture-build","CLI.8.0.0",8);
            AuditStore.Attempt a=store.begin(scope,"used","state.get","{}");store.complete(a,"COMPLETED",map("ok",true),map(),map(),null);
            store.endSession("CLOSED","normal_eof");second=store.beginSession("boot-2","fixture-build","CLI.8.0.0",8);
            assertTrue(store.begin(scope,"used","state.get","{}").duplicate);
            // Simulate a process that never wrote its normal session end.
        }
        try(AuditStore store=new AuditStore(root)){
            store.recoverInterrupted();String third=store.beginSession("boot-3","fixture-build","CLI.8.0.0",8);
    ...
    @Test public void everySessionFreezesItsBuildMetadataAcrossCloseRestartAndHistoryReads()throws Exception{
        ...
        try(AuditStore store=new AuditStore(root)){
            scope=store.menuScope();first=store.beginSession("first-boot","build-first","CLI.8.0.0",8);
        ...
        try(AuditStore store=new AuditStore(root)){
            String second=store.beginSession("second-boot","build-second","CLI.8.0.0",8);
            assertEquals(map("build_id","build-first","cli_version","CLI.8.0.0","protocol_version",8),
            ...
            for(String side:new String[]{"public","internal"})try(Connection db=open(root,side);Statement statement=db.createStatement()){
                ...
                assertEquals("CLI.8.0.0",scalar(db,"SELECT cli_version FROM sessions WHERE session_id='"+first+"'"));
                assertEquals(8,((Number)scalar(db,"SELECT protocol_version FROM sessions WHERE session_id='"+first+"'")).intValue());
    ...
    @Test public void missingBuildMetadataCannotStartOrRecoverASession()throws Exception{
        ...
        try(AuditStore store=new AuditStore(root)){
            assertThrows(IllegalArgumentException.class,()->store.beginSession("boot",null,"CLI.8.0.0",8));
            assertThrows(IllegalArgumentException.class,()->store.beginSession("boot","build",null,2));
            assertThrows(IllegalArgumentException.class,()->store.beginSession("boot","build","CLI.8.0.0",3));
            assertThrows(IllegalArgumentException.class,()->store.beginSession("boot","build","CLI.7.0.1",7));
            assertThrows(IllegalArgumentException.class,()->store.beginSession("boot","build","CLI.8.0.0",9));
    ...
    @Test public void saveReceiptsKeepSuccessFailureScopeAndFollowingObservationWithoutLeakingException()throws Exception{
        Path root=temporary.newFolder().toPath();
        try(AuditStore store=new AuditStore(root)){
            store.beginSession("boot","fixture-build","CLI.8.0.0",8);String menu=store.menuScope(),run="run:example";
            ...
            store.complete(start,"COMPLETED",map("v",8,"s",run,"st","completed","data",map("hero",map("class","warrior"))),map("scene","game","hero",map("class","warrior")),map("hidden",1),null,run);
            for(String name:new String[]{"public","internal"})try(Connection c=open(root,name)){
    ...
    @Test public void schemaThreeIsRejectedWithoutInventingSessionsOrChangingWireBytes()throws Exception{
        ...
        AuditSchemaElevenTest.assertRejectedWithoutChanges(root);
    }
