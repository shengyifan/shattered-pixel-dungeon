...
public class GameLogEventsTest {
    ...
    @Test public void eventsReadDrainsDisplayedSnapshotsBeforeItsResponseAndConsoleRemainsPrivate()throws Exception{
        try(AuditStore store=new AuditStore(temporary.newFolder().toPath())){
            store.ensureScope("run:a","run","a");store.ensureScope("run:old","run","old");store.beginSession("log-test","fixture-build","CLI.6.0.0",6);
            ...
            try(MachineSession session=new MachineSession(store,game,new PrintStream(checked,true,"UTF-8"),1000)){
                ...
                session.accept(V6Requests.encode(store, map("protocol_version", 6, "scope_id","run:a","id","logs","op","events.read"))).get(5,TimeUnit.SECONDS);
                ...
                session.accept(V6Requests.encode(store, map("protocol_version", 6, "scope_id","run:a","id","state","op","state.get"))).get(5,TimeUnit.SECONDS);
                assertEquals("v1",store.resolveHandle("revision",(String)last(wire).get("rev")));
                session.accept(V6Requests.encode(store, map("protocol_version", 6, "scope_id","run:old","id","old","op","events.read"))).get(5,TimeUnit.SECONDS);
    ...
    @Test public void handshakePublishesBuildSessionAndAuditSchemaMetadata()throws Exception{
        try(AuditStore store=new AuditStore(temporary.newFolder().toPath())){
            String sessionId=store.beginSession("handshake-test","fixture-build","CLI.6.0.0",6);ByteArrayOutputStream wire=new ByteArrayOutputStream();
            try(MachineSession session=new MachineSession(store,new FakeGame(),new PrintStream(wire,true,"UTF-8"),1000)){
                session.accept(V6Requests.encode(store, map("protocol_version", 6, "id","hello","op","protocol.info"))).get(5,TimeUnit.SECONDS);
                Map<?,?> result=(Map<?,?>)last(wire).get("data");
                assertEquals(sessionId,store.resolveHandle("session",(String)result.get("session_id")));assertEquals(9,((Number)result.get("audit_schema_version")).intValue());
    ...
    @Test public void consecutiveSourcelessDisplaysRemainPartialEventsWithoutClosingTheSession()throws Exception{
        try(AuditStore store=new AuditStore(temporary.newFolder().toPath())) {
            store.ensureScope("run:a","run","a");store.beginSession("translation-failure","fixture-build","CLI.6.0.0",6);
            FakeGame game=new FakeGame();ByteArrayOutputStream wire=new ByteArrayOutputStream();
            ...
            try(MachineSession session=new MachineSession(store,game,new PrintStream(wire,true,"UTF-8"),1000)) {
                session.accept(V6Requests.encode(store, map("protocol_version", 6, "scope_id","run:a","id","display-query","op","events.read"))).get(5,TimeUnit.SECONDS);
                ...
                session.accept(V6Requests.encode(store, map("protocol_version",6,"scope_id","run:a","id","after-display","op","state.get"))).get(5,TimeUnit.SECONDS);
                assertFalse(last(wire).containsKey("err"));
    ...
    @Test public void visualHistoryRetainsDisappearedCuesAndNeverSerializesTheLevelIdentity()throws Exception{
        try(AuditStore store=new AuditStore(temporary.newFolder().toPath())){
            ...
            try(MachineSession session=new MachineSession(store,game,new PrintStream(wire,true,"UTF-8"),1000)){
                session.accept(V6Requests.encode(store, map("protocol_version", 6, "scope_id","run:a","id","first","op","events.read"))).get(5,TimeUnit.SECONDS);
                game.visuals.add(new GameController.VisualSnapshot("a",identity,1,"opaque-map",2,"2026-09-09T01:00:01Z",List.of()));
                session.accept(V6Requests.encode(store, map("protocol_version", 6, "scope_id","run:a","id","second","op","events.read"))).get(5,TimeUnit.SECONDS);
