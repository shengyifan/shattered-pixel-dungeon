...
class UiTextVisibilityTest {
    ...
    @Test void emptyBitmapInitializationDoesNotInventAnInvisibleIntentChange() {
        try(TextObservationFixture ignored=new TextObservationFixture()) {
            ...
            Map<String,Object> emptyUi=UiDrawFixture.capture(ui).describeUi();
            String emptyIntent=ui.intentSignature();
            String emptyContext=UiDrawFixture.capture(ui).contextSignature();
            text.text("");
            assertEquals(emptyUi,UiDrawFixture.capture(ui).describeUi(),"Null and empty both publish a node without text");
            assertEquals(emptyIntent,ui.intentSignature(),"An undrawn initialization must not stale the next input");
            assertEquals(emptyContext,UiDrawFixture.capture(ui).contextSignature());
            text.text(null);
            assertEquals(emptyIntent,ui.intentSignature());

            text.text(ResourceTextFixture.literal("Visible notice"));
            assertTrue(UiDrawFixture.capture(ui).describeUi().toString().contains("Visible notice"));
    ...
    @Test void nonemptyUnboundTextStillInvalidatesIntentWhenItsTranslationIsUnavailable() {
        try(TextObservationFixture ignored=new TextObservationFixture()) {
            ...
            UiBridge ui=new UiBridge(()->scene);
            assertEquals("partial",PublicEnglishProjection.presentation(UiDrawFixture.capture(ui).describeUi()).get("status"));
    ...
    @Test void clippedBitmapCannotExposeItsSourceOrInvalidateIntentThroughItsHiddenSuffix() {
        ...
        Map<String,Object> initial=UiDrawFixture.capture(ui).describeUi();
        ...
        assertEquals(signature,ui.intentSignature());assertEquals(initial,UiDrawFixture.capture(ui).describeUi());
        text.x=10;text.width=30;
        assertTrue(UiDrawFixture.capture(ui).describeUi().toString().contains("Defense"));
    ...
    @Test void transparentBitmapHasNoPublicTextOrSource() {
        ...
        Map<String,Object> hidden=UiDrawFixture.capture(new UiBridge(()->scene)).describeUi();
    ...
    @Test void styledButtonCannotPublishItsUndrawnCachedBodyThroughAnAggregateShortcut() throws Exception {
        ...
        Map<String,Object> hidden=UiDrawFixture.capture(ui).describeUi();
        assertFalse(hidden.toString().contains("Blocking"));
        assertFalse(hidden.toString().contains("windows.wndupgrade.blocking"));
        body.visible=true;
        Map<String,Object> shown=UiDrawFixture.capture(ui).describeUi();
