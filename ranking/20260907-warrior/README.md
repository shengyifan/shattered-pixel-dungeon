...
└── image/             # 5 张精选压缩图片、README 与完整提示词 JSON
```

## 通关精选插画

[精选插画与逐图说明](image/README.md) 收录用户挑选并压缩的 5 张图片，包含粘咕蓄力、DM-300 电塔战、酸蝎停时门战、锈拳交战与取得 Yendor 护符五个场景。全部为 1:1、1254×1254 PNG，保持用户提供的压缩文件内容。

[完整提示词 JSON](image/prompts.json) 按当前英文文件名关联完整生成提示词、参考素材顺序及实测尺寸和大小。
...
原始通关资料共 **49 个文件**：根目录 1 份 `README.md`，`tmp/` 中 44 个临时工作文件，`memory/` 中 2 份项目摘录、1 份解析器和 1 份清单。

精选图片目录另外收录 **7 个文件**：5 张 PNG、1 份 README 和 1 份完整提示词 JSON。加上原始通关资料的 49 个文件，本归档共收录 **56 个文件**。
