...
public class ControlRequestTest {
    ...
    @Test public void validatesVersionBeforeOtherFields() {
        ...
        assertRejected("INVALID_REQUEST", "{\"v\":6,\"id\":false,\"op\":[]}");
    }
    @Test public void oldEnvelopesOperationsAndArgumentsAreNotAccepted() {
        ...
        assertRejected("UNKNOWN_OPERATION", "{\"v\":6,\"id\":\"x\",\"op\":\"action.execute\"}");
        for(String field:new String[]{"protocol_version","scope_id","state_version","args","action","direction"})
            assertRejected("INVALID_REQUEST", "{\"v\":6,\"id\":\"x\",\"op\":\"move\",\""+field+"\":\"x\"}");
        assertRejected("INVALID_ARGUMENT", "{\"v\":6,\"id\":\"x\",\"op\":\"move\",\"dir\":\"north\"}");
    }
    @Test public void flatOperationsBecomeCanonicalEngineArgumentsWithoutChangingRaw() {
        ControlRequest move=ControlRequest.parse("{\"v\":6,\"id\":\"a\",\"s\":\"run:one\",\"rev\":\"one:9\",\"op\":\"move\",\"dir\":\"NE\"}");
        ...
        for(String op:WireNames.operations()) {
            ControlRequest parsed=ControlRequest.parse(request("6",op));
    ...
    @Test public void optionalFrozenSourcesAndReceiptDetailsAreValidated() {
        ControlRequest query=ControlRequest.parse("{\"v\":6,\"id\":\"q\",\"s\":\"run:one\",\"op\":\"req\",\"rid\":\"a\",\"src\":true,\"get\":[\"reply\",\"after\"]}");
        ...
        assertRejected("INVALID_REQUEST","{\"v\":6,\"id\":\"q\",\"op\":\"state\",\"src\":1}");
        for(String get:new String[]{"null","true","{}","[1]","[\"internal\"]","[\"raw\",\"raw\"]"})
            assertRejected("INVALID_REQUEST","{\"v\":6,\"id\":\"q\",\"op\":\"req\",\"get\":"+get+"}");
        assertRejected("INVALID_REQUEST","{\"v\":6,\"id\":\"q\",\"op\":\"click\",\"src\":true}");
    }
    @Test public void viewsArePresentationOnlyAndSourcesForceFull() {
        for(String op:Arrays.asList("state","actions")) {
            ControlRequest defaults=ControlRequest.parse(request("6",op));
            assertFalse(defaults.fullView);assertTrue(defaults.args.isEmpty());
            for(String view:Arrays.asList("play","full")) {
                ControlRequest query=ControlRequest.parse("{\"v\":6,\"id\":\"q\",\"op\":\""+op+"\",\"view\":\""+view+"\"}");
            ...
            for(String view:Arrays.asList("null","true","1","[]","{}","\"compact\""))
                assertRejected("INVALID_REQUEST","{\"v\":6,\"id\":\"q\",\"op\":\""+op+"\",\"view\":"+view+"}");
        }
        ControlRequest sources=ControlRequest.parse("{\"v\":6,\"id\":\"q\",\"op\":\"state\",\"view\":\"play\",\"src\":true}");
        ...
        for(String op:Arrays.asList("req","move","info"))
            assertRejected("INVALID_REQUEST","{\"v\":6,\"id\":\"q\",\"op\":\""+op+"\",\"view\":\"full\"}");
    }
