...
public class JsonCodecTest {
    ...
    @Test public void parsesOnlyTheRequestEnvelopeWithoutDemandingActiveScope() {
        ControlRequest request = ControlRequest.parse("{\"v\":5,\"id\":\"query-1\",\"op\":\"info\"}");
        assertEquals("query-1", request.id);
        assertNull(request.scopeId);
        assertNull(request.stateVersion);
        assertTrue(request.args.isEmpty());
        ControlRequest move = ControlRequest.parse("{\"v\":5,\"s\":\"run:uuid\",\"id\":\"a\",\"op\":\"move\",\"rev\":\"boot:7\",\"dir\":\"N\"}");
        assertEquals("boot:7", move.stateVersion);
        assertEquals("north", move.args.get("direction"));
        assertThrows(ProtocolException.class, () -> ControlRequest.parse("{\"v\":5,\"op\":\"state\"}"));
        assertThrows(ProtocolException.class, () -> ControlRequest.parse("{\"v\":5,\"id\":\"a\",\"op\":\"move\",\"args\":[]}"));
    }
}
