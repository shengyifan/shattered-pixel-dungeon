...
2026-09-12按用户要求移除旧测试应用、runtime及数据，保留本轮0.8.12验证和正式playthroughs。Git中的历史报告不删除，但旧build路径已不再可读；旧请求/内部快照不再可从那些目录重建。详细范围见[清理记录](cli-test-cleanup-20260912.md)。CLI本身仍默认保留新审计，不进行自动历史删减。
