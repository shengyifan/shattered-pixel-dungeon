...
本文件是实现状态记录。最终实战目标是仅使用包内 `spdctl run --machine` 从主菜单控制战士通关，并覆盖完整返程到地表。用户于 2026-09-09 将六职业分别通关的验收缩减为战士一局；全角色功能和专项覆盖要求继续保留。普通游戏规则、消耗、解锁、随机数和存档流程保持不变。
...
| P9 | 战士纯 CLI 通关，并覆盖完整返程 | 0/1；正式公开 CLI 战士探索进行中，尚未报告通关 |
