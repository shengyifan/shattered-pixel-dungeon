...
> 历史归档：本页保留早期 CLI 实施批次的当时结论，旧协议示例、profile 规则与“当前”用语均指对应历史版本。CLI8 的当前契约见 [CLI 操作手册](cli-help.md)、[CLI8 实施与验证记录](cli8-implementation.md)和[语义战斗观察](cli-combat-visuals.md)；CLI7 的具体历史记录见 [CLI 7 实施记录](cli7-implementation.md)、[CLI.7.0.1 渲染观察](cli7-rendered-combat-visuals.md)及[当时清理重建记录](cli-rebuild-7.0.1-clean-20260923.md)。
