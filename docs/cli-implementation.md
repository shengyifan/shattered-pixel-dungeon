...
本文件是实现状态记录。2026-09-09 用户进一步要求先停止正式实战，改为直接构造中间状态覆盖全部场景；当时战士已正常保存退出。2026-09-12用户进一步授权清空全部本地运行数据，旧战士profile现已删除，计划在CLI.0.9.0重新建立新局实战。原最终实战目标是仅使用包内 `spdctl run --machine` 从主菜单控制战士通关，并覆盖完整返程到地表。用户于 2026-09-09 将六职业分别通关的验收缩减为战士一局；全角色功能和专项覆盖要求继续保留。普通游戏规则、消耗、解锁、随机数和存档流程保持不变。

## 开发测试产物清理

2026-09-12先完成旧产物清理，随后用户明确要求把剩余未纳入Git的运行/构建数据全部移除，为CLI.0.9.0新实战准备。所有build、旧战士存档、audit和策略轨迹现已删除；之前的保留决定已被这条后续指令取代。源码和Git报告保留，原始build路径不可再读。详见[本地数据重置](cli-runtime-reset-20260912.md)；[较早清理记录](cli-test-cleanup-20260912.md)作为历史记录保留。CLI本身的默认审计保留策略未改变。
...
| P8 | 故障、保存、断流、包与性能 | 保存/恢复/强杀/未捕获异常/布局及性能各有分项证据；9月12日当前CLI.0.8.12实际ARM64包已通过英文管道/中文窗口化/保存重启验收，范围见cli-package-english-validation.md |
| P9 | 战士纯 CLI 通关，并覆盖完整返程 | 0/1。旧实战profile已按用户要求清空，计划在CLI.0.9.0使用新profile开展新局实战；新版尚未打包 |
