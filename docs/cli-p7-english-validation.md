...
逐项对应的运行时、build ID、隔离 profile、公开 trace、后置断言与原始批次报告见 完整验证索引（历史 JSON 已删除，可查 Git 历史）。补验索引（历史 JSON 已删除，可查 Git 历史） 单列从失败中补齐的 40 项。原有 [P7 验证记录](cli-p7-validation.md) 保留其历史批次事实；本文件专门验证后续新增的英文 CLI 与简体中文 GUI 组合。
