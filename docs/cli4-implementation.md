...
同日后续按用户要求清理了此前全部 build 产物及实战故障 JSON。本文保留 CLI.4.0.0 实施时的验证结论和复算方法，旧 build 输出、测试依赖及验证附件已清理；当时 4.0.1 包的重建结果见 [CLI.4.0.1 清理重建记录](cli-rebuild-4.0.1-20260914.md)。
