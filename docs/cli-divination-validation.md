...
两项均已完整通过，合计 **37 次**与响应同 `state_version`、同 `scope_id` 的原 GUI 中文窗口化核验。公开 CLI 游戏文案逐响应检查为英文，两项正常退出码均为 0，无清理异常。详细请求、版本、原窗口类与 profile 见 JSON 证据（历史 JSON 已删除，可查 Git 历史）。
