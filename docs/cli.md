...
当前版本 `CLI.0.6.0`，协议 1，游戏 3.3.8。完整实施/未完成事项见 [实施记录](cli-implementation.md)。这是可运行的开发版本，按用户最新要求，正式战士实战已保存退出，当前集中进行构造中间状态的真实引擎场景覆盖。

新增语言约定：游戏GUI使用中文；CLI请求、名称、提示、说明及响应均使用英文。当前已提交版本仍保留旧的界面文本投影，语言解耦正在实现，旧中文实机记录不算新约定的验收。
