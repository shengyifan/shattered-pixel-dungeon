...
当前版本 `CLI.0.8.12`，协议 1，游戏 3.3.8。完整实施/未完成事项见 [实施记录](cli-implementation.md)。这是已验证的源码版本。按用户2026-09-12后续要求，本地构建产物与全部旧运行数据已清空；后续计划打包CLI.0.9.0，使用新profile开始实战。当前不再保留可直接运行的应用包或旧战士续档。
