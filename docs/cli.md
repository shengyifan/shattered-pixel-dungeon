...
本 CLI 基于原版 **Shattered Pixel Dungeon 3.3.8**（上游基线 `7b8b845a7`、游戏版本码 `896`）扩展。当前 CLI 版本 `CLI.1.0.0`，协议版本 `1`；三个版本标识独立。完整实施/未完成事项及本次构建验证见 [实施记录](cli-implementation.md)。2026-09-12按用户要求直接升级至CLI.1.0.0，完成macOS ARM64重建和包验证，取代先前CLI.0.9.0计划。应用位于下方启动说明中的`desktop-control/build/app-macos-arm64/`。旧运行数据已清空；正式实战需要新profile，不能继续已删除的旧战士存档。版本升级不代表已完成全部场景验收或正式战士通关。
