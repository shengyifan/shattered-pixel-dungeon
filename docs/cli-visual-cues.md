...
天狗第一阶段的实际陷阱图案及跨回合替换另见 [FadingTraps 验证](cli-tengu-traps-validation.md)。各批次语言与构建版本按真实证据分别记录。
...
逐例 profile、构建 ID、冻结运行时和旧失败证据见 cli-visual-cues-validation.json（历史 JSON 已删除，可查 Git 历史）。主要中文批次是 `runtime-b33d2f8f4b80448a8778636772cad8f2`；粘咕模态遮挡/恢复补充是 `runtime-cc58c13125854c8a8d8e42d68d087dfc`，使用同一个生产构建。旧英语失败批次仍保留原语言事实，没有改写成中文测试。
