...
- [CLI.6.0.1 控制器终端启动修复](cli-issues/2026-09-20-cli-6.0.1-controller-pty-startup.md)：子进程 stderr 独立管道、共享 PTY 回归、分阶段诊断与当前包验证边界。
...
2026-09-12 整理时的验证：5项Java静态清单检查、4项Python显式轨迹选择回归通过；基线迁移前后字节完全相同，法术/武器/武僧枚举仍为27/32/5。当时文档中的JSON链接全部指向有效测试基线，验证生成的本地构建缓存随后清理。这些是该历史批次的结果，当前启动修复与包验证见上方 CLI.6.0.1 问题记录。
