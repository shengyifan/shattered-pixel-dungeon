...
`docs/cli-issues/` 随 Git 提交 Markdown 问题说明与修复结论，本地原始证据由 `.gitignore` 排除。2026-09-13 的 CLI.3.0.0 清理重建按用户要求删除了此前 build 产物，以及两份 `*-public-evidence.json` 和 `evidence/` 下的一份故障 JSON；相应历史说明中的附件已不在本地。2026-09-14 新提取的金币提示故障证据单独保留在本地 `evidence/`，详情见本轮修复记录。
