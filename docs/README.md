...
2026-09-12 整理时的验证：5项Java静态清单检查、4项Python显式轨迹选择回归通过；基线迁移前后字节完全相同，法术/武器/武僧枚举仍为27/32/5。当时文档中的JSON链接全部指向有效测试基线，验证生成的本地构建缓存随后清理。这些是该历史批次的结果，当前重建验证见上方 CLI.5.0.0 实施与验收。
