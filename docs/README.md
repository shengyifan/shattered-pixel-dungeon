...
- [完整单帧响应的 token 研究](cli-complete-frame-token-study-20260920.md)：每条回复独立完整，同帧共享、表格、可读状态行、地图网格与非游戏元数据边界；未修改生产格式。
...
2026-09-12 整理时的验证：5项Java静态清单检查、4项Python显式轨迹选择回归通过；基线迁移前后字节完全相同，法术/武器/武僧枚举仍为27/32/5。当时文档中的JSON链接全部指向有效测试基线，验证生成的本地构建缓存随后清理。这些是该历史批次的结果，当前改动与包验证见上方 CLI.6.1.1 实战数据分析。
